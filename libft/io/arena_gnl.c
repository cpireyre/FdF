...
/*   Updated: 2024/08/05 11:16:36 by copireyr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ar_list_to_string(t_arena a, t_gnl_list *head, size_t total_size);
static int	find_newline(char *str, int size);
static void	*list_free(t_gnl_list *head);

char	*arena_gnl(t_arena arena, int fd)
{
	t_gnl_list			*head;
	t_gnl_list			*tail;
	size_t				total_size;
	...
	return (ar_list_to_string(arena, head, total_size));
}
...
static char	*ar_list_to_string(t_arena a, t_gnl_list *head, size_t total_size)
{
	...
	if (total_size <= 0)
		return (list_free(head));
	line = arena_alloc(a, total_size + 1);
	if (!line)
		return (list_free(head));
	current_size = 0;
	node = head;
