...
/*   Updated: 2024/06/20 13:52:06 by copireyr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static uint8_t	interpolate_component(uint8_t s, uint8_t e, int cur, int steps)
{
	...
	fixed_steps = to_fixed_point(steps);
	fixed_curr = to_fixed_point(cur);
	return ((uint8_t)to_integer(to_fixed_point(s) + fixed_point_divide(fixed_curr * (e - s), fixed_steps)));
}
