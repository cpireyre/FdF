 ...
 */
void	DrawPixel(mlx_image_t *image, int x, int y, int color)
{
...
void DrawWuLine(mlx_image_t *image, int X0, int Y0, int X1, int Y1, int BaseColor, int NumLevels, unsigned int IntensityBits)
{
   ...
   /*uint64_t	IntensityShift;*/
   uint32_t ErrorAccTemp; 
	/*uint64_t	Weighting, WeightingComplementMask;*/
   ...
   DrawPixel(image, X0, Y0, BaseColor);
   ...
   if (DeltaY > DeltaX) {
      ...
      ErrorAdj = (uint32_t)(((uint64_t) DeltaX << 16) / (uint64_t) DeltaY);
      ...
      while (--DeltaY) {
         ...
         DrawPixel(image, X0, Y0, BaseColor);
         DrawPixel(image, X0 + XDir, Y0, BaseColor);
         /*DrawPixel(image, X0, Y0, BaseColor + Weighting);*/
         /*DrawPixel(image, X0 + XDir, Y0, BaseColor + (Weighting ^ WeightingComplementMask));*/
      }
      /* Draw the final pixel, which is always exactly intersected by the line
         and so needs no weighting */
      DrawPixel(image, X1, Y1, BaseColor);
      return;
   }
   /* It's an X-major line; calculate 16-bit fixed-point fractional part of a
      pixel that Y advances each time X advances 1 pixel, truncating the
      result to avoid overrunning the endpoint along the X axis */
   ErrorAdj = (uint32_t)(((uint64_t) DeltaY << 16) / (uint64_t) DeltaX);
   /* Draw all pixels other than the first and last */
   while (--DeltaX) {
      ErrorAccTemp = ErrorAcc;   /* remember currrent accumulated error */
      ErrorAcc += ErrorAdj;      /* calculate error for next pixel */
      ...
      /*Weighting = ErrorAcc >> IntensityShift;*/
      DrawPixel(image, X0, Y0, BaseColor);
      DrawPixel(image, X0, Y0 + 1, BaseColor);
      /*DrawPixel(image, X0, Y0, BaseColor + Weighting);*/
      /*DrawPixel(image, X0, Y0 + 1, BaseColor + (Weighting ^ WeightingComplementMask));*/
   }
   /* Draw the final pixel, which is always exactly intersected by the line
      and so needs no weighting */
   DrawPixel(image, X1, Y1, BaseColor);
